    ...
    <section className="py-32 bg-[#0d0d18] border-t border-white/5">
      <div className="max-w-5xl mx-auto px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          >
            <p className="font-inter text-white/30 text-xs tracking-widest uppercase mb-6">Without Us</p>
            <div className="space-y-4">
              {theyHandle.map((item, i) => (
                ...
                >
                  ...
                  <p className="font-inter text-white/40 text-sm line-through decoration-red-500/30">{item}</p>
                </motion.div>
              ))}
            </div>
          </motion.div>
          ...
          <motion.div
            ...
            viewport={{ once: true }}
            transition={{ duration: 0.6, ease: [0.22, 1, 0.36, 1] }}
            className="rounded-2xl p-8 bg-[#0d1a33] border border-[#1A6EFF]/20 will-change-transform"
          >
            <p className="font-inter text-[#1A6EFF] text-xs tracking-widest uppercase mb-6">With Halftime Hustle</p>
            <div className="space-y-4">
              {weHandle.map((item, i) => (
                <motion.div
                  key={i}
                  ...
                  className="flex items-center gap-3 will-change-transform"
                >
                  <div className="w-5 h-5 rounded-full bg-[#1A6EFF]/20 border border-[#1A6EFF]/40 flex items-center justify-center flex-shrink-0">
                    <svg width="10" height="10" viewBox="0 0 10 10" fill="none">
                      <path d="M2 5l2.5 2.5L8 2" stroke="#1A6EFF" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
                    </svg>
                  </div>
                  <p className="font-inter text-white text-sm font-medium">{item}</p>
                </motion.div>
              ))}
            </div>
          </motion.div>
